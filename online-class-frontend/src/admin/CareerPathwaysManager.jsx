    ...
    try {
      ...
      if (docFile) {
        ...
        try {
          ...
          if (upRes.data?.data?.file_url) {
            finalFileUrl = upRes.data.data.file_url;
          }
        } catch {
          if (!finalFileUrl) {
            finalFileUrl = URL.createObjectURL(docFile);
          }
        }
      }
    ...
    try {
      ...
      if (vidFile) {
        try {
          ...
          const upRes = await api.post("/admin/items", uploadData);
          if (upRes.data?.data?.file_url) {
            finalVideoUrl = upRes.data.data.file_url;
          }
        } catch {
          if (!finalVideoUrl) {
            finalVideoUrl = URL.createObjectURL(vidFile);
          }
        }
      }
      ...
      if (vidThumbnail) {
        try {
          ...
          thumbData.append("title", `${vidFormData.title.trim()} Thumbnail`);
          thumbData.append("file", vidThumbnail);
          const thumbRes = await api.post("/admin/items", thumbData);
          if (thumbRes.data?.data?.file_url) {
            finalThumbUrl = thumbRes.data.data.file_url;
          }
        } catch {
          if (!finalThumbUrl) {
            finalThumbUrl = URL.createObjectURL(vidThumbnail);
          }
        }
      }
